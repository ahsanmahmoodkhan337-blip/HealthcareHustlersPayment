...
export function generateReceiptPDF(data: ReceiptPDFData): Promise<Uint8Array> {
  ...
  const docDefinition: any = {
    ...
    footer: {
      columns: [
        {
          text: 'Healthcare Hustlers — Empowering Healthcare Professionals',
          alignment: 'center',
          fontSize: 8,
          color: '#666',
          margin: [0, 10, 0, 0],
        ...
        columns: [
          {
            ...
            stack: [
              ...
              {
                text: 'Registered under SECP',
                fontSize: 9,
                color: '#555',
                margin: [0, 0, 0, 4],
              },
              {
                text: 'www.healthcarehustlers.org',
                fontSize: 9,
                color: '#2980b9',
      ...
      },
      {
        table: {
          ...
          body: [
            ...
            [
              { text: 'Amount Paid' },
              { text: `PKR ${data.amountPaid.toLocaleString()}`, bold: true },
            ],
          ],
        },
        layout: {
              ...
              table: {
                ...
                body: [
                  ...
                  [
                    { text: 'Balance Due' },
                    {
                      text: `PKR ${(data.remainingAmount ?? 0).toLocaleString()}`,
                      bold: true,
                      color: '#c0392b',
                    },
                  ],
                  [
                    { text: 'Next Due Date' },
                    {
                      text: data.nextDueDate
                        ? new Date(data.nextDueDate).toLocaleDateString('en-PK', {
                            year: 'numeric',
                            month: 'long',
                            day: 'numeric',
                          })
